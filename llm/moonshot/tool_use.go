...
func parseToolResponse(body string, _ *zap.Logger) (*models.LLMResponse, error) {
	...
	// Shared OpenAI-compatible parser: marks the data as real API usage
	// (IsReal) and surfaces cached/reasoning token details — the hand-rolled
	// block it replaces silently dropped IsReal, making /cost label real
	// counts as "character estimate".
