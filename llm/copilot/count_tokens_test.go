...
func TestCopilotCountTokens_GPTLocal(t *testing.T) {
	c := NewClient(auth.NewStaticTokenProvider("k", auth.AuthModeAPIKey, auth.ProviderID("x")), "gpt-5.6-terra", zap.NewNop(), 1, time.Millisecond)
