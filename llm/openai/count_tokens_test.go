...
func TestOpenAICountTokens_GPTLocal(t *testing.T) {
	c := NewOpenAIClient(auth.NewStaticTokenProvider("k", auth.AuthModeAPIKey, auth.ProviderID("x")), "gpt-5.6-terra", zap.NewNop(), 1, time.Millisecond)
