...
func (c *OpenRouterClient) SendPrompt(ctx context.Context, prompt string, history []models.Message, maxTokens int) (string, error) {
	...
	start := time.Now()
	client.LogRequestStart(c.logger, "OPENROUTER", c.model, client.CallerField(ctx),
	...
	if err != nil {
		client.LogRequestFinish(c.logger, "OPENROUTER", c.model, "error", time.Since(start), client.CallerField(ctx))
		c.logger.Error(i18n.T("llm.error.get_response_after_retries", "OpenRouter"), zap.Error(err))
		return "", err
	}

	client.LogRequestFinish(c.logger, "OPENROUTER", c.model, "success", time.Since(start), client.CallerField(ctx),
