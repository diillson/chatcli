...
// DevinStaticRate returns the static Cognition rate for a Devin model id,
// family slug or variant uid, applying the fast/priority rate when the
// suffix asks for it. ok=false for a family the listing never priced. It
// is the last resort behind the override and the account's own listing.
func DevinStaticRate(model string) (inputPerMTok, outputPerMTok float64, ok bool) {
	return devinStaticPricing(model)
}

// devinStaticPricing is DevinStaticRate.
