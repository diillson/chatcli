...
func defaultDiagnosticAllowlist() map[string]bool {
	return map[string]bool{
		...
		"nslookup kube-dns.kube-system.svc.cluster.local":    true,
		"getent hosts kubernetes.default.svc.cluster.local":  true,
		"getent ahosts kubernetes.default.svc.cluster.local": true,
		"dig +short kubernetes.default.svc.cluster.local":    true,
		"dig +short +search kubernetes.default":              true,
		"host kubernetes.default.svc.cluster.local":          true,

		// Local health / metrics / debug endpoints (common K8s sidecar conventions)
		"curl -s localhost/health":                             true,
		"curl -s localhost/healthz":                            true,
		"curl -s localhost/ready":                              true,
		"curl -s localhost/readyz":                             true,
		"curl -s localhost/live":                               true,
		"curl -s localhost/livez":                              true,
		...
		"curl -s localhost:8080/live":                          true,
		"curl -s localhost:8080/livez":                         true,
		"curl -s localhost:8080/metrics":                       true,
		"curl -s localhost:8081/metrics":                       true,
		"curl -s localhost:9090/-/ready":                       true,
		"curl -s localhost:9090/-/healthy":                     true,
		"curl -s localhost:9090/metrics":                       true,
		"curl -s localhost:9091/metrics":                       true,
		"curl -s localhost:15000/ready":                        true, // Envoy admin
		"curl -s localhost:15020/healthz/ready":                true, // Istio sidecar
		"curl -s localhost:15021/healthz/ready":                true, // Istio healthz
		"curl -s localhost:6060/debug/pprof/":                  true, // Go pprof index
		"curl -s localhost:6060/debug/pprof/goroutine?debug=1": true,
		"curl -s localhost:6060/debug/pprof/heap?debug=1":      true,

		// Alpine images often lack curl but ship wget
		"wget -qO- http://localhost/health":       true,
		"wget -qO- http://localhost/healthz":      true,
		"wget -qO- http://localhost:8080/health":  true,
		"wget -qO- http://localhost:8080/healthz": true,
		"wget -qO- http://localhost:8080/metrics": true,

		// TCP reachability probes to well-known in-cluster endpoints
		"nc -zv kubernetes.default.svc.cluster.local 443":  true,
		"nc -zv kube-dns.kube-system.svc.cluster.local 53": true,
	}
}
