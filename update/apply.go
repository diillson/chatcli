...
// (com ou sem prefixo "v"; só o self-replace a utiliza — brew e go install
// resolvem "latest" sozinhos). Erros tipados deste pacote descrevem cada
// modo de falha; ErrManualMethod cobre os canais sem atualização automática.
func Apply(ctx context.Context, info Info, latestTag string, opts Options) error {
	switch info.Method {
	case MethodHomebrew:
		return runTool(ctx, opts, BrewUpgradeArgs())
	case MethodGoInstall:
		return runTool(ctx, opts, GoInstallArgs())
