...
- CrashLoopBackOff with app-level bug (nil dereference, panic, unrecoverable startup error): if a prior healthy revision exists, RollbackDeployment. If revision 1 (no history) and the failure is deterministic (AIInsight identifies an app/config bug), use ScaleDeployment with replicas=0 AND containment="true" to stop the crashloop and escalate — restarting or scaling up only amplifies noise and blocks the kubelet. Do NOT propose ExecDiagnostic + ScaleDeployment>0 in this case; the diagnostic cannot fix the bug.
- Missing dependency (connection refused to a Service that does not exist): diagnostics (getent/dig/nc) only confirm; remediation is out of the operator's toolset (user must create the dep). Plan ScaleDeployment replicas=0 containment="true" to stop the bleeding and escalate. Do not retry.
- Pending pod (scheduling failure): diagnostic-only from the operator side — no action reliably fixes insufficient resources or node selector mismatch. ScaleDeployment=0 containment="true" if it is blocking a bigger rollout.
...
- Rollback feasibility: RollbackDeployment only works when the Deployment has a prior ReplicaSet with a healthy image. For first-generation workloads (revision=1), rollback is not viable — pick ScaleDeployment=0 containment="true" or AdjustResources instead.

Action ordering rules:
- ScaleDeployment with replicas=0 is safety-blocked by default; you MUST pass params.containment="true" alongside when using it as intentional stop-the-bleeding for an unrecoverable app bug. Without that flag the plan fails with "scaling to 0 replicas is not allowed (destructive)". Same flag applies to ScaleStatefulSet replicas=0.
- When AIInsight confidence is high (≥0.80) and the root cause is already identified in the analysis, put the REMEDIATION action FIRST, not a diagnostic. Diagnostics only make sense when the root cause is ambiguous — once the AIInsight is confident, an ExecDiagnostic in position 0 that gets rejected (because the command is not in the exact-match allowlist) aborts the plan before the remediation ever runs. Order: remediation → (optional) post-remediation diagnostic → verification.
- If a previous attempt's failure context contains "not in approved diagnostic commands whitelist", the previous ExecDiagnostic command is rejected by the allowlist. Either pick a command that is literally in the allowlist for the next attempt, or drop the diagnostic entirely and rely only on the remediation step.`)
