...
func (cli *ChatCLI) GetInternalCommands() []prompt.Suggest {
	return []prompt.Suggest{
		...
		{Text: "/update", Description: i18n.T("complete.root.update")},
		...
		{Text: "/storage", Description: i18n.T("complete.root.storage")},
		...
		{Text: "/web", Description: i18n.T("help.command.web")},
...
func (cli *ChatCLI) getConfigSuggestions(d prompt.Document) []prompt.Suggest {
	...
	if len(args) == 1 || (len(args) == 2 && !strings.HasSuffix(line, " ")) {
		sections := []prompt.Suggest{
			...
			{Text: "update", Description: i18n.T("complete.config.update")},
			{Text: "memory", Description: i18n.T("cfg.section.memory.title")},
	...
	// /config image <TAB> → mutating subcommands + values
	if s := strings.ToLower(args[1]); s == "image" || s == "img" {
		return cli.getConfigImageSuggestions(d)
	}
