...
var envDefaults = map[string]envDefault{
	...
	"DEVIN_CLI_RESPECT_WORKSPACE_TRUST": {Value: "false", IsBool: true, Source: "config.DevinCLIDefaultRespectWorkspaceTrust (turns run in a throwaway temp dir the CLI cannot prompt about; true restores the CLI default)"},
	...
	"CHATCLI_AGENT_REFUSAL_FALLBACK": {Value: "auto", Source: "agent_refusal.refusalFallbackHandle — auto = sibling model of the same provider (Fable→Opus, Opus→Sonnet); off = plain resend; PROVIDER:model = as given"},
	...
	"CHATCLI_PROMPT_CACHE_TTL":             {Value: "5m", Source: "llm/client/cache_prefix.go (5m|1h|auto; auto = 5m until the session watches its prefix expire during a pause the hour would cover, then 1h; keep-alive refreshes cover long tool calls)"},
	"CHATCLI_PROMPT_CACHE_KEEPALIVE":       {Value: "auto", Source: "cli/cache_keepalive.go (auto = refresh the cache entry while a run's tool call outlives it on every adapter that can, and while idle on models whose cache reads cost at most 5% of input, e.g. Fable 5.1; on = also while idle on every adapter that can; off)"},
	...
	"CHATCLI_EMBED_PROVIDER":      {Value: "(off)", Source: "llm/embedding/factory.go (empty == null; voyage|openai|google|bedrock|ollama)"},
	"CHATCLI_KNOWLEDGE_RERANK":    {Value: "off", Source: "cli/knowledge_rerank.go (off|mmr|llm; rerank stage over hybrid knowledge retrieval)"},
	"CHATCLI_KNOWLEDGE_NORMALIZE": {Value: "off", Source: "cli/ctxmgr/lexical_normalize.go (off|fold|stem; accent folding and light pt/en stemming for the BM25 index; changing it reindexes)"},
	"CHATCLI_EMBED_MODEL":         {Value: "(provider-specific)", Source: "voyage-4 / text-embedding-3-small / gemini-embedding-2 / amazon.titan-embed-text-v2:0 / nomic-embed-text (ollama, OLLAMA_HOST)"},
	...
	"CHATCLI_ENCRYPTION_KEY":                {Value: "(unset — masked when set)", Source: "pkg/atrest (opts the process into encryption at rest; ≥32 random bytes recommended, shorter secrets are stretched with Argon2id)"},
	"CHATCLI_AUDIT_LOG_PATH":                {Value: "", Source: "llm_audit.go (absolute path of the hash-chained audit trail; LLM and gRPC entries share it)"},
	"CHATCLI_COMPRESSION":                   {Value: "lossy-with-ccr", Source: "compress/layer.go NewLayerFromEnv (off | lossless | lossy-with-ccr; /config compression mode changes it live)"},
	"CHATCLI_COMPRESSION_PROFILE":           {Value: "default", Source: "compress/profile.go ProfileDefault (conservative | default | aggressive; /config compression profile changes it live)"},
	"CHATCLI_COMPRESSION_THRESHOLD":         {Value: "4000", Source: "compress/layer.go DefaultThreshold (bytes; smaller tool outputs pass through verbatim)"},
	"CHATCLI_COMPRESSION_CCR_DIR":           {Value: "~/.chatcli/ccr", Source: "compress/layer.go (archive of the full outputs the lossy strategies summarized)"},
	"CHATCLI_COMPRESSION_CCR_MAX_MB":        {Value: "256", Source: "compress/layer.go DefaultCCRMaxMB"},
	"CHATCLI_COMPRESSION_CCR_TTL":           {Value: "168h", Source: "compress/layer.go DefaultCCRTTL (7 days)"},
	"CHATCLI_MEMORY_RETENTION_DAYS":         {Value: "30", Source: "workspace/memory DefaultConfig.DailyNoteRetention"},
	"CHATCLI_MEMORY_MAX_FACTS":              {Value: "500", Source: "workspace/memory DefaultConfig.MaxFactsCount"},
	...
	"CHATCLI_CHAT_CODER_HANDOFF":            {Value: "true", IsBool: true, Source: "cli/coder_handoff.go (the assistant proposes switching to coder mode; the user confirms at the prompt)"},
	...
	"CHATCLI_KEYCHAIN_BACKEND":       {Value: "auto", Source: "auth.NewKeychainStore (auto|keychain|file)"},
	...
	"CHATCLI_SERVER_PIPELINE":        {Value: "false", IsBool: true, Source: "cmd/server_pipeline.go (host the full ChatCLI turn engine behind ChatTurn, RunCoder, RunAgent and tool RPCs)"},
