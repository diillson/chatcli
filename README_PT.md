...
| **Ferramentas** | `/watch {pid\|file}` · `/worktree {create,list,remove}` · `/channel {create,switch}` · `/websearch <query>` · `/lsp <arquivo>` — mais os builtins de agente `@browser`, `@forge`, `@view`, `@proc` (background + PTY interativo), `@taskgraph` (orquestração em DAG verificada, acompanhe com `/taskgraph`), e os subcomandos do `@coder` `outline`/`map`/`checkpoint` |
