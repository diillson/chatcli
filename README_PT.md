...
| **Checkpoints shadow-git** | Snapshot automático do workspace antes de todo subcomando de mutação do `@coder`, num repo git sombra sob `~/.chatcli` — o seu `.git` nunca é tocado. `@coder checkpoint {list,create,restore}` rebobina edições multi-arquivo e efeitos de exec. `CHATCLI_CODER_CHECKPOINTS=off`. |
