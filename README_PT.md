...
| | |
|---|---|
| **Multi-provider com fallback** | 14 provedores de LLM (OpenAI · OpenAI Responses · Anthropic · Bedrock · Google · xAI · ZAI · MiniMax · Moonshot (Kimi) · Copilot · StackSpot · OpenRouter · Ollama · Devin CLI), com classificação inteligente de erros, backoff exponencial e cooldown por provider. |
...
#### Z.AI GLM Coding Plan (assinatura)

- `ZAI_USE_CODING_PLAN=true` aponta o provider ZAI para o endpoint da assinatura (`https://api.z.ai/api/coding/paas/v4/chat/completions`). A **mesma** API key do platform vale nos dois endpoints — é o caminho `/coding/` que faz as requests debitarem do plano em vez dos créditos pay-as-you-go.
- O plano serve `glm-5.3` e `glm-5.3-flash`; requests para GLMs mais antigos são roteadas para eles no servidor. O `/cost` reporta essas chamadas a $0: o uso é coberto pela assinatura, não faturado por token.
- Uma `ZAI_API_URL` explícita sempre vence o toggle — configure a URL completa de chat completions (ex.: o endpoint mainland `https://open.bigmodel.cn/api/coding/paas/v4/chat/completions`). Qualquer URL com caminho `/api/coding/` é tratada como uso do plano.
- `ZAI_THINKING=enabled|disabled` controla o modo thinking do GLM nos caminhos normal e de tool calling (sem definir, mantém o default do backend; `enabled` também preserva o reasoning intercalado entre turnos).
...
| **Ferramentas** | `/watch {pid\|file}` · `/worktree {create,list,remove}` · `/channel {create,switch}` · `/websearch <query>` · `/lsp <arquivo>` — mais os builtins de agente `@browser`, `@forge`, `@view`, `@proc` (background + PTY interativo), `@taskgraph` (orquestração em DAG verificada, acompanhe com `/taskgraph`), a dash ao vivo do runtime inteiro `/dash`, e os subcomandos do `@coder` `outline`/`map`/`checkpoint` |
...
| **Automação de navegador (@browser)** | Dirige um Chrome/Chromium/Brave/Edge local real via DevTools Protocol — falado direto pelo websocket client já embarcado, **sem dependência nova, sem driver, sem chave**. `open`/`snapshot` (página como texto com elementos interativos numerados)/`click`/`type`/`press`/`hover`/`select`/`upload`/`eval`/`screenshot` (`--full`)/`html`/`pdf`/`resize`/`tabs`/`cookies`/`console`/`network`. O loop de verificação para trabalho web: constrói um frontend, então o VÊ e depura pelo console/network. **Passagem de bastão visível**: `show` coloca a janela na tela do usuário para ele mesmo logar, `wait` bloqueia até a página chegar a uma URL/texto/elemento, `hide` volta ao headless — o perfil (logins) sobrevive à troca. `CHATCLI_BROWSER_BIN`, `CHATCLI_BROWSER_HEADLESS`, `CHATCLI_BROWSER_PROFILE` (logins persistentes), `CHATCLI_BROWSER_CDP_URL` (controla o Chrome do próprio usuário). |
| **Git forge (@forge)** | Pull requests, issues e CI pelo `gh`/`glab` que o PRÓPRIO usuário autenticou — **sem chave**, auto-detectado pelo remote git. `pr-list/view/diff/checks/create/comment`, `issue-*`, `ci-status`, `ci-logs` (logs dos steps que falharam). Fecha o loop branch → PR → CI → fix sem sair do agente; leituras auto-aprovadas, mutações no security gate. |
| **Entrada de imagem (@view)** | O agente OLHA uma imagem local no meio da tarefa — um screenshot do `@browser`, um mock, um diagrama — pelo pipeline de visão (multimodal nativo ou describe-fallback), estagiada e anexada no próximo turno. |
| **Task graph (@taskgraph)** | Um plano multi-task aprovado vira um DAG persistido: tasks independentes rodam em paralelo em squad workers, dependências controlam a execução, e *done nunca é a autodeclaração do executor* — o engine executa os comandos de validação de cada task ele mesmo e um reviewer worker de contexto limpo dá o veredito (done ou retry com feedback). Custo real por task via usage do provider, checkpoint shadow-git antes de cada executor, runs retomáveis em `~/.chatcli/taskgraph/`. Humanos acompanham mid-run via `/taskgraph` (status, evidência por task, cancel) ou pela **dashboard live no browser** (`/taskgraph dash`): canvas de DAG animado com swimlanes por fase, pan/zoom, popovers por task com evidência do reviewer e saídas do gate, critical path e custo real por task — servida read-only em 127.0.0.1, fechar nunca afeta a run. Vale a partir de ~5+ tasks genuinamente independentes. |
| **Dash ao vivo (/dash)** | Um grafo em tempo real do runtime inteiro no navegador, em **todos os processos chatcli da máquina** (REPL, gateway, ACP dentro da IDE, servidor MCP, daemon do scheduler): agents em árvore pelo pai real, e hubs para modelos de LLM, tools, skills, servidores MCP, conexões de saída, trabalho em background (scheduler, `@proc`, memory worker, compaction, LSP, `@browser`) e os **sete padrões do harness com o que cada um concluiu** (`rewrote draft`, `found discrepancy`, `lesson saved`, …). Cada requisição de LLM fica pendurada no agent que a fez; o card da sessão mostra o par que atende a próxima requisição e segue toda troca (`@model use`, hint de modelo de skill, `/switch`, `/provider`, gateway), cada uma virando uma linha no feed. Cada faixa de processo carrega custo, requisições, ocupação do contexto e contagem de erros; o cabeçalho tem badge de erros do último minuto, busca que esmaece o resto e sparkline de eventos por segundo; o feed filtra por kind ou só erros, mostra as marcações de fase que o agent deixa com `@dash mark`, e todo hub de modelo mostra o acerto do prompt cache; o spool inteiro exporta como NDJSON. A página tem um visual único e deliberado, um terminal escuro (a paleta da apresentação do ChatCLI): uma janela estilo macOS por processo, tiles de estatística em mono grande, um feed `$ tail -f` e uma barra de status; só um tema de terminal claro a reveste de outro jeito. Desligada por padrão e gratuita enquanto desligada (um load atômico por ponto instrumentado); a gravação segue um lease que a dash só mantém vivo enquanto um navegador está olhando, ou `CHATCLI_DASH=1` desde o boot. Só metadados: nunca prompts, argumentos ou saída de tool, caminhos de URL, linhas de comando. Servida em 127.0.0.1 com token de uso único e checagem de Host exato; somente leitura, então fechar nunca afeta uma sessão. `/dash` funciona mid-run e via ACP; `chatcli dash` serve a partir de um terminal próprio. |
| **Recuperação de recusa** | Uma resposta que o classificador de segurança do provider interrompe (Anthropic `stop_reason: refusal`, OpenAI `content_filter`) não encerra mais a sessão do coder como "nenhuma resposta". O erro nomeia a causa — inclusive a categoria de `stop_details` do classificador (`cyber`, `bio`, `reasoning_extraction`) e a explicação, quando a Anthropic as dá; o loop avisa o modelo que nada rodou e reenvia o turno no modelo que o provider **recomenda** para aquela categoria, senão num **modelo irmão do mesmo provider** (Fable → Opus, Opus → Sonnet; `CHATCLI_AGENT_REFUSAL_FALLBACK=auto|off|PROVIDER:model`) e volta ao seu modelo no turno seguinte. Uma execução recusada mais de duas vezes fica no fallback até o fim. A troca aparece como mudança de rota na dash ao vivo (`via refusal fallback`). |
| **Tool da dash (@dash)** | O agent gerencia a dash ao vivo como o usuário faz com `/dash` e lê o que ela vê: `open`/`url` sobem a dash e devolvem o endereço, `off` para, `status` diz quem está gravando, `summary` renderiza o grafo reduzido deste processo ou de todos os processos chatcli (modelo, rota e custo da sessão, agents, requisições de LLM por modelo com tokens e custo, tools, skills, servidores MCP, padrões, trabalho em background, conexões, erros recentes), `events` devolve os eventos crus mais recentes filtráveis por kind e status, e `mark` deixa um rótulo de fase na linha do tempo. Só metadados, nunca conteúdo. Desliga com `CHATCLI_AGENT_DASH_TOOL=false`. |
| **Sandbox de exec** | Confinamento opt-in no nível do SO para `@coder exec`: `CHATCLI_CODER_SANDBOX=off\|workspace\|strict\|docker`. `sandbox-exec` nativo (macOS) / `bwrap` (Linux) / **container Docker-Podman em QUALQUER SO incl. Windows**; `strict` ainda corta a rede. Confina o que um comando permitido pode tocar, sobre o validator e a policy que gateiam o que roda. |
| **Checkpoints shadow-git** | Snapshot automático do workspace antes de todo subcomando de mutação do `@coder`, num repo git sombra sob `~/.chatcli` — o seu `.git` nunca é tocado. `@coder checkpoint {list,create,restore}` rebobina edições multi-arquivo e efeitos de exec. Snapshots são limitados: workspaces amplos como o seu diretório home nunca recebem snapshot automático, cada snapshot roda sob um deadline rígido (`CHATCLI_CODER_CHECKPOINT_TIMEOUT` segundos), e falhas repetidas fazem backoff e desativam snapshots na sessão. `CHATCLI_CODER_CHECKPOINTS=off`. |
| **Diagnósticos pós-edição** | Após um write/patch bem-sucedido do `@coder`, os diagnósticos do language server dos arquivos tocados são injetados no resultado da tool — uma edição quebrada é pega turnos antes de um teste falhar. `CHATCLI_CODER_AUTODIAG=off`. Mais `@coder outline`/`map` para orientação estrutural do código (repo map via `go/ast`) sem ler arquivos inteiros. |
| **Processos interativos (@proc pty)** | `@proc start {pty:true}` roda um programa num pseudo-terminal real (REPLs, debuggers, `ssh`) e `@proc stdin` digita nele — programas que exigem TTY não travam mais o agente (Unix-first). |
...
| **Curadoria de prefixo (chat)** | O prefixo do chat paga os mesmos bytes a cada turno, então o que é estável vai para o prefixo cacheado e o que é volumoso vai para trás de um pull. A metade do workspace que não depende da pergunta (arquivos de bootstrap, índice de memória, card do grafo de conhecimento) passa a ser cacheada em vez de reenviada por turno; uma skill auto-ativada cujo corpo já está na conversa avisa isso em vez de mandá-lo de novo (e volta a ser embutido no instante em que aquela cópia sai do histórico, ou quando a skill muda em disco); o catálogo MCP é listado por nome. Nada fica inalcançável: a tool `context_pull` é a terceira exceção sancionada do chat (read-only, local — corpo de skills, catálogo de skills instaladas, catálogo MCP completo com descrições), e todo adiamento cita a chamada exata que traz o conteúdo de volta. Desligar (`/config chat pull`, `CHATCLI_CHAT_CONTEXT_PULL`) desarma a curadoria e volta a embutir tudo, porque um resumo que o modelo não consegue expandir é perda de capacidade, não economia. O TTL do prompt cache segue a mesma regra de evidência: a entrada de 1h custa 2x a escrita, então só é pedida depois que a sessão viu o próprio prefixo expirar numa pausa que a hora teria coberto. |
