...
| **Ferramentas** | `/watch {pid\|file}` · `/worktree {create,list,remove}` · `/channel {create,switch}` · `/websearch <query>` · `/lsp <arquivo>` — mais os builtins de agente `@browser`, `@forge`, `@view`, `@proc` (background + PTY interativo), e os subcomandos do `@coder` `outline`/`map`/`checkpoint` |
