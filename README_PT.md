...
| **Automação de navegador (@browser)** | Dirige um Chrome/Chromium/Brave/Edge local real via DevTools Protocol — falado direto pelo websocket client já embarcado, **sem dependência nova, sem driver, sem chave**. `open`/`snapshot` (página como texto com elementos interativos numerados)/`click`/`type`/`eval`/`screenshot`/`console`/`network`. O loop de verificação para trabalho web: constrói um frontend, então o VÊ e depura pelo console/network. `CHATCLI_BROWSER_BIN`, `CHATCLI_BROWSER_HEADLESS`. |
