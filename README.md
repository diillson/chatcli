...
| **Tools** | `/watch {pid\|file}` · `/worktree {create,list,remove}` · `/channel {create,switch}` · `/websearch <query>` · `/lsp <file>` — plus agent builtins `@browser`, `@forge`, `@view`, `@proc` (background + interactive PTY), and `@coder` subcommands `outline`/`map`/`checkpoint` |
...
| **Shadow-git checkpoints** | An automatic workspace snapshot before every mutating `@coder` subcommand, into a shadow git repo under `~/.chatcli` — your own `.git` is never touched. `@coder checkpoint {list,create,restore}` rewinds multi-file edits and exec side effects. `CHATCLI_CODER_CHECKPOINTS=off`. |
