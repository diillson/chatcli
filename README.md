...
| **Knowledge base (RAG keyless)** | `/context create docs corpus.jsonl --mode knowledge` — corpora de docs (ex.: JSONL do `@docs-flatten`) viram base de conhecimento: o attach injeta só um index card (~900 tokens fixos, mesmo com 6MB+) e trechos relevantes são recuperados por turno via BM25 puro-Go (sem API key) + embeddings quando configurados. No agent/coder, a tool `@knowledge` (search/get/toc) consulta a base iterativamente — inclusive pra criar skills a partir da doc com `@skill`. |
