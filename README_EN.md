...
| **Mixture-of-Agents** | `/moa` — several models propose in parallel and an aggregator synthesizes (Wang et al., 2406.04692). |
...
| **Knowledge base (keyless RAG)** | `/context create docs corpus.jsonl --mode knowledge` — documentation corpora (e.g. `@docs-flatten` JSONL) become a knowledge base: attaching injects only an index card (~900 fixed tokens, even at 6MB+) and relevant passages are retrieved per turn via pure-Go BM25 (no API key) + embeddings when configured. In agent/coder, the `@knowledge` tool (search/get/toc) interrogates the base iteratively — including authoring skills from the docs with `@skill`. |
